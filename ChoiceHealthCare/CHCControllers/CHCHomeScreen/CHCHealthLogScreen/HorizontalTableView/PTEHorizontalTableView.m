...
//    self.tableView.frame = CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height);
    self.tableView.frame = CGRectMake(0, 0, 10 , 20);
    // Adjust frame
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
   ...
   // return tableView.rowHeight;
    return self.bounds.size.width/1.3;
}
...
{
    if ([self.delegate respondsToSelector:@selector(numberOfSectionsInhorizontalTableView:)])
    {
        return [self.delegate numberOfSectionsInhorizontalTableView:self];
    }
    return 1;
}
